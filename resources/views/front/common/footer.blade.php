<div id="footer"
     style="background-image: url({{url('images/artificial-intelligence-ai-robot-big-data-bull-market-stock-chart-getty.jpg')}})">
    <div class="background-overlay">
        <div class="container py-100px">
            <div class="row">
                <div class="col-md-3 footer-first-item">
                    <div class="logo-footer">
                        <a href="{{route('front.home.index')}}">
                            <img height="80px" src="{{asset('images/2-1024x359.png')}}"
                                 alt="{{asset('images/2-1024x359.png')}}">
                        </a>
                    </div>
                    <div class="social-media">
                        <a href="">
                            <i class="fa fa-facebook" aria-hidden="true"></i>
                        </a>
                        <a href="">
                            <i class="fa fa-youtube-play" aria-hidden="true"></i>
                        </a>
                        <a href="">
                            <i class="fa fa-twitter" aria-hidden="true"></i>
                        </a>
                    </div>
                </div>
                <div class="col-md-3">
                    <h5 class="text-white text-bold text-26px mb-4"> Chúng tôi</h5>
                    <ul class="custom-ul-li">
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Về chúng tối</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Hệ thống của chúng tôi</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Dịch vụ</span>
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="col-md-3">
                    <h5 class="text-white text-bold text-26px mb-4"> Liên kết hữu ích</h5>
                    <ul class="custom-ul-li">
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Hiệu quả gia dịch</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Cách hệ thống vận hành</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Nghiên cứu đầu tư</span>
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="col-md-3">
                    <h5 class="text-white text-bold text-26px mb-4"> Thông tin liên lạc</h5>
                    <ul class="custom-ul-li">
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Phường An Khánh, Tp.Thủ Đức, HCM</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>+84961201343</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>info@yoursite.com</span>
                            </a>
                        </li>
                        <li class="custom-ul-li">
                            <a href="#" class="text-white">
                                <i class="fa fa-angle-right custom-icon" aria-hidden="true"></i>
                                <span>Mon - Sat 09:00 - 17:00</span>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="footer-bottom">
    Copyright 2023 © GVN-Fintrade
</div>